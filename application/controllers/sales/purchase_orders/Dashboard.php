...
class Dashboard extends Sales_Controller {
	...
	/**
	 * Clear Linesheet History
	 *
	 * Primary method to call when no other methods are found in url segment
	 *
	 * @return	void
	 */
	public function clear_linesheet_history()
	{
		echo 'Clearing...';
		...
		// grab options and extract linesheet_history
		$sales_user_options = $this->sales_user_details->options;
		unset($sales_user_options['linesheet_history']);

		$DB->set('options', json_encode($sales_user_options));
		$DB->where('admin_sales_id', $this->sales_user_details->admin_sales_id);
		$query = $DB->update('tbladmin_sales');

		$this->session->set_flashdata('success', 'clear_linesheet_history');

		// redirect...
		redirect('sales/dashboard', 'location');
	}
