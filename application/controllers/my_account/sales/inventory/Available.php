...
class Available extends Sales_user_Controller {
	...
	public function index()
	{
		...
		// this controller is for sales users only
		// for categories, we check conditions of site type
		if ($this->sales_user_details->designer == 'shop7thavenue')
		{
		...
		$this->data['active_designer'] =
			...
			? (
				$this->sales_user_details->designer == 'shop7thavenue'
				? FALSE
		...
		else
		{
			if ( ! $this->input->post('search_string'))
			{
				...
				{
					redirect('my_account/sales/inventory/available/index/womens_apparel/dresses/evening_dresses');
				}
				else
				{
					redirect('my_account/sales/inventory/available/index/womens_apparel/dresses/evening_dresses');
		...
		if ($this->input->post('search_string'))
		{
			...
			$where['tbl_product.prod_no LIKE'] = $this->input->post('search_string');
		...
		$this->data['page_title'] = 'Inventory Available';
