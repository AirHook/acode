...
class Store_credit extends Sales_user_Controller {
	...
	public function index($list = 'all', $des_slug = '')
	{
		...
		$this->data['page_title'] = 'Store Credit Orders';
	...
	private function _set_pagination($count_all = '', $per_page = '', $list, $des_slug)
	{
		...
		if ($des_slug && !is_numeric($des_slug))
		{
			$ref_uri = 'admin/orders/store_credit/index/'.$list.'/'.$des_slug;
		}
		else $ref_uri = $list == 'all' ? 'admin/orders/store_credit' : 'admin/orders/store_credit/index/'.$list;

		$config['base_url'] = base_url().'admin/orders/store_credit/index/'.($list ? $list.'/' : '').(($des_slug && !is_numeric($des_slug)) ? $des_slug.'/' : '');
