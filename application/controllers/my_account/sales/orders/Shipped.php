...
class Shipped extends Sales_user_Controller {
	...
	public function index($list = 'all', $des_slug = '')
	{
		...
		$where['status'] = '1'; // complete/shipped
		...
		$this->data['page_title'] = 'Shipped and Complete';
	...
	private function _set_pagination($count_all = '', $per_page = '', $list, $des_slug)
	{
		...
		if ($des_slug && !is_numeric($des_slug))
		{
			$ref_uri = 'admin/orders/shipped/index/'.$list.'/'.$des_slug;
		}
		else $ref_uri = $list == 'all' ? 'admin/orders/shipped' : 'admin/orders/shipped/index/'.$list;

		$config['base_url'] = base_url().'admin/orders/shipped/index/'.($list ? $list.'/' : '').(($des_slug && !is_numeric($des_slug)) ? $des_slug.'/' : '');
