...
class Refunded extends Sales_user_Controller {
	...
	private function _set_pagination($count_all = '', $per_page = '', $list, $des_slug)
	{
		...
		if ($des_slug && !is_numeric($des_slug))
		{
			$ref_uri = 'admin/orders/refunded/index/'.$list.'/'.$des_slug;
		}
		else $ref_uri = $list == 'all' ? 'admin/orders/refunded' : 'admin/orders/refunded/index/'.$list;

		$config['base_url'] = base_url().'admin/orders/refunded/index/'.($list ? $list.'/' : '').(($des_slug && !is_numeric($des_slug)) ? $des_slug.'/' : '');
