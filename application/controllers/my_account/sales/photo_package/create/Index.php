...
class Index extends Sales_user_Controller {
	...
	public function index()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->session->sa_pp_des_slug = $des_slug;
			...
			if ($this->session->sa_pp_des_slug)
			{
				// get designer details
				$this->data['designer_details'] = $this->designer_details->initialize(
					array(
						'url_structure' => $this->session->sa_pp_des_slug
					)
				);
				...
				$this->data['des_subcats'] = $this->categories_tree->treelist(
					array(
						'd_url_structure' => $this->session->sa_pp_des_slug,
						'with_products' => TRUE
					)
				);
				$this->data['row_count'] = $this->categories_tree->row_count;
				$this->data['max_level'] = $this->categories_tree->max_category_level;
				$this->data['primary_subcat'] = $this->categories_tree->get_primary_subcat($this->session->sa_pp_des_slug);

				// get last category slug
				if ($this->session->sa_pp_slug_segs)
				{
					$this->data['slug_segs'] = json_decode($this->session->sa_pp_slug_segs, TRUE);
				...
				{
					$designer_slug = $this->session->sa_pp_des_slug;
			...
			$this->data['page_breadcrumb'] = array(
				'sales_package' => 'Photo Packages',
				'create' => 'Create'
			);

			// set data variables...
			$this->data['role'] = 'sales';
			$this->data['file'] = 'sa_pp_create';
			$this->data['page_title'] = 'Photo Package';
			$this->data['page_description'] = 'Create Photo Packages';
		...
		else
		{
			...
			if ($this->input->post('save_sales_package'))
			{
				...
				$post_ary['options']['des_slug'] = $this->session->sa_pp_des_slug; // additional data
				$post_ary['options']['designers'] = $this->session->sa_pp_designers;
				...
				$post_ary['sales_package_items'] = $this->session->sa_pp_items ?: json_encode($post_ary['prod_no']);
				...
				unset($_SESSION['sa_pp_des_slug']);
				unset($_SESSION['sa_pp_designers']);
				...
				// redirect user
				redirect('my_account/sales/photo_package/send/index/'.$insert_id);
			}
			else
			{
				...
				$_SESSION['sa_pp_email_subject'] = $this->input->post('email_subject');
				$_SESSION['sa_pp_email_message'] = $this->input->post('email_message');
				$_SESSION['sa_pp_options'] = json_encode($this->input->post('options'));

				// redirect user
				redirect('my_account/sales/photo_package/send_package', 'location');
			}
		}
	}
	...
	private function _create_plugin_scripts()
	{
		...
		$this->data['page_level_scripts'] = '';
			...
			$this->data['page_level_scripts'].= '
				<script src="'.base_url().'assets/custom/js/metronic/pages/scripts/sales-pp-components.js?z='.time().'" type="text/javascript"></script>
			';
	}
