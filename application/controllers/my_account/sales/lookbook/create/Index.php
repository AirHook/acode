...
class Index extends Sales_user_Controller {
	...
	public function index()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->session->lb_des_slug = $des_slug;
			$this->session->lb_designers = array($des_slug);
			...
			if ($this->session->lb_des_slug)
			{
				...
				$this->data['designer_details'] = $this->designer_details->initialize(
					array(
						'url_structure' => $this->session->lb_des_slug
					)
				);
				...
				$this->data['des_subcats'] = $this->categories_tree->treelist(
					array(
						'd_url_structure' => $this->session->lb_des_slug,
						'with_products' => TRUE
					)
				);
				$this->data['row_count'] = $this->categories_tree->row_count;
				$this->data['max_level'] = $this->categories_tree->max_category_level;
				$this->data['primary_subcat'] = $this->categories_tree->get_primary_subcat($this->session->lb_des_slug);
		...
		else
		{
			...
			if ($this->input->post('save_lookbook'))
			{
				...
				$post_ary['options']['des_slug'] = $this->session->lb_des_slug;
				if ($this->session->lb_designers) $post_ary['options']['designers'] = json_encode($this->session->lb_designers);
				...
				$post_ary['designer'] = $this->session->lb_des_slug;
				...
				$post_ary['items'] = $this->session->lb_items;
				...
				// redirect user
				redirect('my_account/sales/lookbook/send/index/'.$insert_id);
			}
			else
			{
