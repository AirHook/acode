...
class Get_category_tree extends MY_Controller {
	...
	public function index()
	{
		...
		if ($this->input->post('slugs_link'))
		{
			...
			//$this->session->set_userdata('lb_slug_segs', $this->input->post('slugs_link'));
		}
		...
		$des_slug =
			$page == 'create'
			? $this->session->lb_des_slug
			: (
				$this->session->lb_mod_des_slug
		...
		if ($page == 'create')
		{
			$this->session->set_userdata('lb_slug_segs', json_encode($slug_segs));
		}
		else
		{
			$this->session->set_userdata('lb_mod_slug_segs', json_encode($slug_segs));
		}
