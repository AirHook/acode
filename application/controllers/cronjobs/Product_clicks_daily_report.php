...
class Product_clicks_daily_report extends MY_Controller {
	...
	public function index()
	{
		...
		$this->data['date'] = @date('Y-m-d', strtotime('yesterday'));
		...
		else
		{
			...
			$this->email->subject($this->webspace_details->name.' - Daily Product Clicks');
		...
		if ($click_data_length >= 1000000)
		{
			$message_to_admin = '
				<br /><br />
				Product Clicks current data size is now at '.$click_data_length.'
			';
			...
			$this->email->subject($this->webspace_details->name.' - Product Clicks Data Size');
