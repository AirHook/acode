...
class Suspended extends Admin_Controller {
	...
	public function index($des_slug = '')
	{
		...
		$this->sales_users_list->pagination = $this->data['page'];
		...
		{
			$params['admin_sales_designer'] = $des_slug ?: '';
		}
		$params['is_active'] = '2';
		$this->data['users'] = $this->sales_users_list->select($params);
		$this->data['count_all'] = $this->sales_users_list->row_count;
	...
	private function _set_pagination($count_all = '', $per_page = '', $des_slug)
	{
		...
		$config['base_url'] = base_url().'admin/users/sales/suspended/index/'.($des_slug ? $des_slug.'/' : '');
