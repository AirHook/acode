...
class Suspended extends Admin_Controller {
		...
		if (@$this->webspace_details->options['site_type'] != 'hub_site')
		{
			$this->data['users'] = $this->wholesale_users_list->select(
				array(
					...
					'tbluser_data_wholesale.is_active' => '2'
				),
				array(),
			...
			array('tbluser_data_wholesale.is_active'=>'2'),
			array(),
			array($this->data['offset'], $this->data['limit'])
		);
	...
	private function _set_pagination($count_all = '', $per_page = '')
	{
		...
		$config['base_url'] = base_url().'admin/users/wholesale/suspended/index/';
		...
		$config['first_url'] = site_url('admin/users/wholesale/suspended');
