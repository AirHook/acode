...
class Edit extends Admin_Controller {
	...
	public function index($id = '')
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if ( ! $this->admin_user_details->initialize(array('admin_id' => $id)))
			{
				// set flash data
			...
			if ( ! $this->session->flashdata('clear_recent_admin'))
			{
				// update recent list for admin users edited
				$this->webspace_details->update_recent_users(array(
					'user_type' => 'admin_users',
					'user_id' => $this->admin_user_details->admin_id,
					'user_name' => $this->admin_user_details->fname.' '.$this->admin_user_details->lname
				));
			}
			...
			// set data variables...
			$this->data['file'] = 'users_admin_edit';
			$this->data['page_title'] = 'Admin User Edit';
			$this->data['page_description'] = 'Edit Admin User Details';
		...
		else
		{
			...
			$query = $this->DB->update('tbladmin', $post_ary);

			// set flash data
			$this->session->set_flashdata('success', 'edit');

			redirect($this->config->slash_item('admin_folder').'users/admin/edit/index/'.$id, 'location');
		}
	}
	...
	public function clear_recent()
	{
		...
		if (
			isset($options['recent_admin_users'])
			&& ! empty($options['recent_admin_users'])
		) unset($options['recent_admin_users']);
		...
		// set flash data
		$this->session->set_flashdata('clear_recent_admin', TRUE);
	...
	private function _create_plugin_scripts()
	{
		...
		$this->data['page_level_scripts'] = '';
			...
			// handle datatable
			$this->data['page_level_scripts'].= '
				<script src="'.base_url().'assets/custom/js/metronic/pages/scripts/form-validation-users_admin_edit.js" type="text/javascript"></script>
			';
	}
