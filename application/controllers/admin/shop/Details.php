...
class Details extends Frontend_Controller
{
	...
	function index()
	{
		...
		if ($prod_list_seq[0] > 1)
		{
			...
			$this->data['prev_link'] =
				'shop/details/'
				.$prev_row->d_url_structure . '/'
				. $prev_row->prod_no. '/'
				. str_replace(' ','-',strtolower(trim($prev_row->color_name))). '/'
				. str_replace(' ','-',strtolower(trim(($prev_row->prod_name ?: $prev_row->prod_no)))). '/'
				. ($this->uri->segment(7) - 1).'of'.$prod_list_seq[1]
			;
		}
		if ($prod_list_seq[0] < $prod_list_seq[1])
		{
			...
			$this->data['next_link'] =
				'shop/details/'
				.$next_row->d_url_structure . '/'
				. $next_row->prod_no. '/'
				. str_replace(' ','-',strtolower(trim($next_row->color_name))). '/'
				. str_replace(' ','-',strtolower(trim(($next_row->prod_name ?: $next_row->prod_no)))). '/'
				. ($this->uri->segment(7) + 1).'of'.$prod_list_seq[1]
			;
		}
	...
	private function _get_details()
	{
		...
		*/
		...
		$this->data['img_front'] =
			$this->product_details->primary_img
			? $this->config->item('PROD_IMG_URL').$this->product_details->media_path.$img_name.'_f2.jpg'
			: $this->config->item('PROD_IMG_URL').$img_path.'product_front/thumbs/'.$img_name.'_2.jpg'
		;
		$this->data['img_side'] =
			$this->product_details->primary_img
			? $this->config->item('PROD_IMG_URL').$this->product_details->media_path.$img_name.'_s2.jpg'
			: $this->config->item('PROD_IMG_URL').$img_path.'product_side/thumbs/'.$img_name.'_2.jpg'
		;
		$this->data['img_back'] =
			$this->product_details->primary_img
			? $this->config->item('PROD_IMG_URL').$this->product_details->media_path.$img_name.'_b2.jpg'
			: $this->config->item('PROD_IMG_URL').$img_path.'product_back/thumbs/'.$img_name.'_2.jpg'
		;
