...
class New_arrival extends Admin_Controller {
	...
	public function index()
	{
		...
		if ($this->webspace_details->options['site_type'] != 'hub_site')
		{
			...
			if ($this->designer_details->initialize(array('designer.url_structure'=>$url_segs[0])))
			{
				if ($this->designer_details->slug != $this->webspace_details->slug)
				{
					...
					redirect('admin/products/new_arrival', 'location');
				}
			}
		...
		$this->data['view_as'] = $this->session->userdata('view_as') ?: 'products_grid'; // products_grid, products_list, products_nestable_list
		...
		else
		{
			...
			)
			{
				$redirect_url =
					$prev_url_segs
					? 'admin/products/new_arrival/index'.$prev_url_segs
					: (
						$this->webspace_details->slug == 'shop7thavenue'
						? 'admin/products/new_arrival/index/basixblacklabel/womens_apparel/dresses/evening_dresses'
						: 'admin/products/new_arrival/index/womens_apparel'
					)
				;
			}
			else
		...
		// new arrival
		// the CAST function is taken from Products_list library file
		// as aliases are not used on the query condition clauses like WHERE
		$con_new_arrival = "(
			(tbl_product.new_arrival='Yes' OR tbl_product.new_arrival='yes' OR tbl_product.new_arrival='y' OR tbl_product.new_arrival='Y' OR tbl_product.new_arrival='New Arrival')
			OR
			(CAST(tbl_product.prod_date as date) >= DATE_ADD(NOW(), INTERVAL -90 DAY))
		)";
		$where['condition'][] = $con_new_arrival;
