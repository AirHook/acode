...
class Dsco_stocks extends Admin_Controller {
	...
	public function index()
	{
		...
		if ($this->webspace_details->options['site_type'] != 'hub_site')
		{
			...
			if ($this->designer_details->initialize(array('designer.url_structure'=>$url_segs[0])))
			{
				if ($this->designer_details->slug != $this->webspace_details->slug)
				{
					...
					redirect('admin/products/dsco_stocks', 'location');
				}
			}
		...
		else
		{
			...
			)
			{
				$redirect_url =
					...
					: (
						...
						? 'admin/products/dsco_stocks/index/basixblacklabel/womens_apparel/dresses/evening_dresses'
						: 'admin/products/dsco_stocks/index/womens_apparel'
					)
				;
			}
			else
			{
				$redirect_url =
					$prev_url_segs
					? 'admin/products/dsco_stocks/index'.$prev_url_segs
					: 'admin/products/dsco_stocks/index/womens_apparel'
				;
			}
		...
		$where['tbl_stock.options LIKE'] = '"post_to_dsco"';
