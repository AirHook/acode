...
class Not_public extends Admin_Controller {
	...
	public function index()
	{
		...
		if ($this->webspace_details->options['site_type'] != 'hub_site')
		{
			...
			if ($this->designer_details->initialize(array('designer.url_structure'=>$url_segs[0])))
			{
				if ($this->designer_details->slug != $this->webspace_details->slug)
				{
					...
					redirect('admin/products/not_public', 'location');
				}
			}
		...
		else
		{
			...
			)
			{
				$redirect_url =
					$prev_url_segs
					? 'admin/products/not_public/index'.$prev_url_segs
					: (
						$this->webspace_details->slug == 'shop7thavenue'
						? 'admin/products/not_public/index/basixblacklabel/womens_apparel/dresses/evening_dresses'
						: 'admin/products/not_public/index/womens_apparel'
					)
				;
			}
			else
	...
	private function _set_pagination($count_all = '', $per_page = '')
	{
		...
		$config['first_url'] = site_url('admin/products/not_public/index/womens_apparel');
