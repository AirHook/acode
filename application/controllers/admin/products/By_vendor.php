...
class By_vendor extends Admin_Controller {
	...
	public function index()
	{
		...
		// we don't set url flash data for be_vendor list
		...
		if ($this->webspace_details->options['site_type'] != 'hub_site')
		{
			...
			if ($this->designer_details->initialize(array('designer.url_structure'=>$url_segs[0])))
			{
				if ($this->designer_details->slug != $this->webspace_details->slug)
				{
					...
					redirect('admin/products/by_vendor', 'location');
				}
			}
		}
		else
		{
			// check any previous url segs for unwanted designer slugs
			if (@$url_segs[0] && $this->designer_details->initialize(array('designer.url_structure'=>$url_segs[0])))
			{
				// unset any prev_url_segs
				unset($_SESSION['prev_url_segs']);
		...
		$this->data['pre_link'] = implode('/', array_diff($uri_string, $url_segs)); //.'/index';
		...
		else
		{
			$this->data['vendors'] = $this->vendor_users_list->select(
				array(
					'vendors.reference_designer' => $this->webspace_details->slug
				)
			);
			/*
			$this->data['categories'] = $this->categories_tree->treelist(
		...
		// let's get primary vendor code
		$primary_vendor = '6ave';
		$_vendors = $this->data['vendors'];
		foreach ($_vendors as $_vendor)
		{
			$primary_vendor = $_vendor->vendor_code;
			break;
		}
		...
		if (count($url_segs) > 0)
		{
			...
			*/
			...
			// last segment as category slug (vendor code)
			$this->data['active_vendor'] =
				is_numeric(end($url_segs))
				? $this->data['url_segs'][count($this->data['url_segs']) - 2]
				: end($url_segs)
			;
			//$this->session->set_userdata('active_vendor', $this->data['active_vendor']);
		}
		else
		{
			$redirect_url = 'admin/products/by_vendor/index/'.strtolower($primary_vendor);
		...
		if ($this->vendor_user_details->initialize(array('vendor_code'=>$this->data['active_vendor'])))
		{
			$vendor_id = $this->vendor_user_details->vendor_id;
			$this->data['active_vendor_id'] = $vendor_id;
		}
		else
		{
			$vendor_id = '';
			$this->data['active_vendor_id'] = '';
		}
		...
		if ($vendor_id)
		{
			$where['tbl_product.vendor_id'] = $vendor_id;
		}
		...
		$this->data['page_param'] = 'by_vendor';
