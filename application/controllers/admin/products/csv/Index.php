...
class Index extends Admin_Controller {
	...
	public function index()
	{
		...
		{
			if ($this->session->csv_uri_string) redirect ($this->session->csv_uri_string, 'location');
			else redirect($this->config->slash_item('admin_folder').'products/csv/index/basixblacklabel/womens_apparel');
		}
		else
		{
			if (
				$this->session->csv_uri_string
				&& $this->session->csv_uri_string == $this->uri->uri_string()
			)
			{
				// keep the csv_uri_string session again
				$this->session->keep_flashdata('csv_uri_string');
			}
			else $this->session->set_flashdata('csv_uri_string', $this->uri->uri_string());
		}
		...
		// above code on csv_uri_string loop alleviates needing to check for active designer and category
		...
		else
		{
			...
			redirect($this->config->slash_item('admin_folder').'products/csv/index/basixblacklabel/womens_apparel');
		}
		...
		// set the filename for the csv file downloading
		// check if filename exist to enable link at front end
		// using designer and categories as reference
		$path = './csv/products/';
		$csv_filename = 'products_'.$this->data['active_designer'].'_'.$this->data['active_category'];
		$this->data['csv_filename'] =
			file_exists($path.$csv_filename.'.csv')
			? $csv_filename
			: FALSE
		;
		...
		$this->data['products'] = $this->products_list_csv->select($where);
		$this->data['products_count'] = $this->products_list_csv->row_count;

		// need to show loading at start
		$this->data['show_loading'] = TRUE;

		// set data variables...
		$this->data['file'] = 'products_csv';
		$this->data['page_title'] = 'Products CSV';
		$this->data['page_description'] = 'CSV Manage Products';
	...
	private function _create_plugin_scripts()
	{
		...
		$this->data['page_level_scripts'] = '';
			...
			$this->data['page_level_scripts'].= '
				<script src="'.base_url().'assets/custom/js/metronic/pages/scripts/table-datatables-product_list_csv.js" type="text/javascript"></script>
			';
	}
