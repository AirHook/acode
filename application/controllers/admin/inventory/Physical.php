...
class Physical extends Admin_Controller {
	...
	public function index()
	{
		...
		array_shift($this->data['url_segs']); // physical
		...
		else
		{
			if ( ! $this->input->post('search_string'))
			{
				...
				if ($this->webspace_details->options['site_type'] == 'hub_site')
				{
					redirect($this->config->slash_item('admin_folder').'inventory/physical/index/basixblacklabel/womens_apparel/dresses/evening_dresses');
				}
				else
				{
					redirect($this->config->slash_item('admin_folder').'inventory/physical/index/'.$this->webspace_details->slug);
				}
			}
		}
		...
		$this->data['inv_prefix'] = 'physical';

		// set data variables...
		$this->data['file'] = 'inventory';
		$this->data['page_title'] = 'Inventory Physical';
