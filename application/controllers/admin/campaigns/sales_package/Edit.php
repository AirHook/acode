...
class Edit extends Admin_Controller {
	...
	public function step2($id = '')
	{
		...
		array_shift($this->data['url_segs']); // campaigns
		...
		else
		{
			...
			redirect($this->config->slash_item('admin_folder').'campaigns/sales_package/edit/step2/'.$id.'/womens_apparel');
			//$this->data['active_designer'] = FALSE;
		...
		if ($this->data['active_designer'])
		{
			$where = array(
				'designer.url_structure' => $this->data['active_designer'],
				'OR tbl_product.categories LIKE' => '%'.$category_id.'%',
			);
		}
		else
		{
