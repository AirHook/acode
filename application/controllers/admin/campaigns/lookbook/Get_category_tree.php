...
class Get_category_tree extends MY_Controller {
	...
	public function index()
	{
		...
		if ($this->input->post('slugs_link'))
		{
			...
			//$this->session->set_userdata('admin_lb_slug_segs', $this->input->post('slugs_link'));
		}
		...
		$des_slug =
			$page == 'create'
			? $this->session->admin_lb_des_slug
			: (
				$this->session->admin_lb_mod_des_slug
		...
		if ($page == 'create')
		{
			$this->session->set_userdata('admin_lb_slug_segs', json_encode($slug_segs));
		}
		else
		{
			$this->session->set_userdata('admin_lb_mod_slug_segs', json_encode($slug_segs));
		}
