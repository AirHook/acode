var TableDatatablesManaged = function () {
    ...
    // get the "data-set" attribute and look for each (look for all rows checkboxes)
    // if header checkbos is checked, check row checkboxes and set row as active
    $('#tbl-admin_users_').find('.group-checkable').change(function () {
        var set = jQuery(this).attr("data-set");
        var checked = jQuery(this).is(":checked");
        jQuery(set).each(function () {
            if (checked) {
    ...
    // table row checkboxes change function
    $('#tbl-admin_users_').on('change', 'tbody tr .checkboxes', function () {
        $(this).parents('tr').toggleClass("active");
        // add other custom javascripts here...
        var checked = jQuery(this).is(":checked");
        if (checked) {
	...
	$('.apply_filer_by_designer').click(function(){
		...
		if (!z) {
            ...
            $('#loading').modal('show');
            if (z=='all'){
                window.location.href = base_url + "admin/users/admin/" + page_param + ".html";
            }else{
                window.location.href = base_url + "admin/users/admin/" + page_param + "/index/" + z + ".html";
            }
		}
	});
