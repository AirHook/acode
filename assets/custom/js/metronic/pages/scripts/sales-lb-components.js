var ComponentsEditors = function () {
    ...
    var handleScripts = function () {
        ...
        // tooltip binging to dynamic content
        $('body').tooltip({
            selector: '[data-tooltips="tooltips"]'
        });
        ...
        function loadPreset(objectData){
            ...
            load_preset.done(function(data){
                if (data === 'loggedout'){
                    ...
                    $('[name="lookbook_name"]').val(data.lookbook_name);
        ...
        function addRemItem(objectData){
            ...
            addrem.done(function(data) {
                ...
                if (objectData.action == 'rem_item'){
                    if (data == 0){
                        $('.thumb-tile.'+objectData.style_no).removeClass('selected');
                        // user toastr notification
                        toastr.info('Item removed...');
                    }
                } else {
                    $('.step3').addClass('active');
                    $('.thumb-tile.'+objectData.style_no).addClass('selected');
                    // user toastr notification
        ...
        function setItems(objectData){
            ...
            set_size_qty.done(function(data){
                if (data === 'loggedout'){
                    ...
                    // other itesm
                    var items_count = $('[name="items_count"]').val();
                    $('.span_items_count').html("("+items_count+")");
                }
            });
        ...
        $('.thumb-tiles-wrapper').on('click', '.package_items', function() {
            ...
            objectData.style_no = $(this).data('item');
            objectData.page = $(this).data('page');
            objectData.category_slug = $(this).data('category_slug');
        ...
        // size and qty change at modal submit
        $('#form-size_qty_select').submit(function(e){
            ...
            if (items_count >= 30) {
                alert('You have reached max 30 items only.');
                return false;
            }
            ...
            objectData.color_name = $('#size-select-color_name').val();
            objectData.category = $('#size-select-category').val();
            objectData.price = $('#size-select-price').val();
        ...
        $('.radio-options').on('change', function(){
            ...
            if (objectData.param == 'w_prices'){
                if (objectData.val == 'Y'){
                    $('.lb-items-w_prices').removeClass('display-none');
                }else{
                    $('.lb-items-w_prices').addClass('display-none');
                }
            }
            if (objectData.param == 'w_sizes'){
                if (objectData.val == 'Y'){
                    $('.lb-items-w_sizes').removeClass('display-none');
                }else{
                    $('.lb-items-w_sizes').addClass('display-none');
                }
            }
            // set new info
        ...
        $('.thumb-tiles.sales-package').on('click', '.package_items', function(){
            var r = confirm('Do you want to remove item from list?');
            if (r) {
                var objectData = object_data;
                objectData.prod_no = $(this).data('prod_no');
                objectData.style_no = $(this).data('item');
    ...
    var handleValidation1 = function() {
		...
		var form1 = $('#form-lb_create_summary_review');
