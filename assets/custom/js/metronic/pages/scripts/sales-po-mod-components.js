    ...
    var handleScripts = function () {
        ...
        */
        ...
        $('.cart_basket_wrapper table tbody').on('click', '.edit_vendor_prices', function(){
            ...
            var prod_no = $(this).data('prod_no'); // <prod_no>
            var item = $(this).data('item'); // <prod_no>_<color_code>
            var this_size_qty = $('.this-total-qty').val();
            var el = $('#vendor_price-'+item);
            var new_price = el.val();
            var page = el.data('page');
            objectData.vendor_price = new_price;
            objectData.prod_no = item;
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    //$('.modal-input-vendor-price.'+prod_no).val(data);
                    // update the unit vendor price and other related variant
                    $('[name="vendor_price-'+item+'"]').val(data);
                    $('.unit-vendor-price.'+item).html(data);
                    // update each variant's subtotal
                    $('.order-subtotal.'+item).each(function(){
                        var thisQty = $(this).siblings('.size-and-qty-wrapper').find('.this-total-qty').val();
                        $(this).html('$ '+(parseFloat(thisQty * data)).toFixed(2));
            ...
            */
			...
			submitHandler: function (form) {
                ...
                if (must_return) {
                    ...
                    bootbox.dialog({
                        message: "Modifying purchase order. Please confirm...<br />This also sends an email to vendor.<br />A copy will also be sent to you.",
                        title: "Notice",
                        buttons: {
                            danger: {
                                label: "Cancel",
    ...
    var handleBootbox = function() {
        $('.step.step4_, .sidebar-send-po-btn_, .submit-po_summary_review_').on('click', function(){
            ...
            if ($(this).hasClass('mt-bootbox-new_')) {
                bootbox.dialog({
                    message: "Modifying purchase order. Please confirm...<br />This sends an email to vendor.<br />A copy will also be sent to you.",
                    title: "Notice",
                    buttons: {
                        danger: {
                            label: "Cancel",
