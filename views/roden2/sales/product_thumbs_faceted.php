                                ...
                                <div class="col col-12of12">
									...
									<div class="v-product-browsepagetemplate">
										...
										<div class="browseoptions browseoptions-top clearfix">
											...
											<?php
											if ($this->uri->uri_string() != 'search_products')
											{
												$this->load->view($this->config->slash_item('template').'sales/product_thumbs_filter');
											}
											else
											{
												echo '<p style="margin:0 0 20px 5px;">Search results for: <strong>'.$prod_no_string.'</strong></p>';
											}
											?>
												...
												<div class="products clearfix">
													...
													?>
														...
														foreach ($view_pane_sql->result() as $thumb):
															...
															$img_thumb 	    = $img_url.'product_front/thumbs/'.$thumb->prod_no.'_'.$thumb->color_code.'_4.jpg';
															$img_thumb_back = $img_url.'product_back/thumbs/'.$thumb->prod_no.'_'.$thumb->color_code.'_4.jpg';
															$img_thumb_side = $img_url.'product_side/thumbs/'.$thumb->prod_no.'_'.$thumb->color_code.'_4.jpg';
															$img_video 		= $img_url.'product_video/'.$thumb->prod_no.'_'.$thumb->color_code.'.flv';
															$img_linesheet 	= $img_url.'product_linesheet/'.$thumb->prod_no.'_'.$thumb->color_code.'.jpg';
															...
															$img = ENVIRONMENT == 'development_' ? @GetImageSize($local_path.str_replace('/','\\',$img_thumb)) : checkRemoteImage($this->config->item('PROD_IMG_URL').$img_thumb);
															$img2 = ENVIRONMENT == 'development_' ? @GetImageSize($local_path.str_replace('/','\\',$img_thumb_back)) : checkRemoteImage($this->config->item('PROD_IMG_URL').$img_thumb_back);
															$img3 = ENVIRONMENT == 'development_' ? @GetImageSize($local_path.str_replace('/','\\',$img_thumb_side)) : checkRemoteImage($this->config->item('PROD_IMG_URL').$img_thumb_side);
															...
															$thumblinesheet = $this->config->item('PROD_IMG_URL').$img_linesheet;
															
															$seg = (($this->uri->segment(1) === 'special_sale' OR $thumb->custom_order === '3') ? 'special_sale/' : '').$thumb->d_url_structure . '/' . $thumb->prod_no. '/' . str_replace(' ','-',strtolower(trim($thumb->color_name))).'/' . str_replace(' ','-',strtolower(trim($thumb->prod_name)));
															
															// the item coloricon / swatch
															$color_icon = $this->config->item('PROD_IMG_URL').$img_url.'product_coloricon/'.$thumb->prod_no.'_'.$thumb->color_code.'.jpg';
														?>
													...
													<div class="product hoverable">
														...
														<div class="image">
															 ...
															 */
															?>
															<div class="primary" onmouseover="$('.alt-image-<?php echo $thumb->prod_no.'_'.$thumb->color_code; ?>').hide();" onmouseout="$('.alt-image-<?php echo $thumb->prod_no.'_'.$thumb->color_code; ?>').show();">
																...
																<?php
																$value = $thumb->prod_no.'='.$thumb->color_code.'='.$thumb->prod_name.'='.$thumb->wholesale_price.'='.$thumb->colors.'='.$thumb->color_name.'='.$img_url.'='.$thumb->clearance.'='.$this->session->userdata('admin_sales_designer');
																?>
													
																<a href="<?php echo $thumblinesheet; ?>" title="<?php echo $thumb->prod_name; ?>" class="sa_thumbs_group" data-alias="">
																	...
																	<img class="alt-image alt-image-<?php echo $thumb->prod_no.'_'.$thumb->color_code; ?>" src="<?php echo $thumbnail; ?>" alt="<?php echo $thumb->prod_name; ?>" style="position:absolute;left:0;top:0;" />
																...
																<?php
																/**********
																 * OPTIONS top corner box
																 */
																?>
																<div class="thumb_select_options">
																
																	<input type="checkbox" name="package_item[]" id="<?php echo $value; ?>" value="<?php echo $value; ?>" <?php echo isset($checked[$thumb->prod_no.'_'.$thumb->color_code]) ? $checked[$thumb->prod_no.'_'.$thumb->color_code] : ''; ?> onclick="go_spin();sa_add_to_cart('<?php echo site_url('sa/update_cart'); ?>', '<?php echo $value; ?>');" title="Select/Deselect Item" />
																	<br />
																	
																	<div id="fav_btn_<?php echo $thumb->prod_no.'_'.$thumb->color_code; ?>">
																	
																	<?php if ($favorites && in_array($thumb->prod_no.'_'.$thumb->color_code, $favorites)): ?>
																	
																	<img src="<?php echo base_url('images/btn_remove_20.png'); ?>" title="Remove from Favorites" onclick="go_spin();sa_rem_favorites('<?php echo site_url('sa/favedits'); ?>', '<?php echo $thumb->prod_no.'_'.$thumb->color_code; ?>', 'favrem');" />
																	
																	<?php else: ?>
																	
																	<img src="<?php echo base_url('images/btn_check_20.png'); ?>" title="Add to Favorites" onclick="go_spin();sa_add_favorites('<?php echo site_url('sa/favedits'); ?>', '<?php echo $thumb->prod_no.'_'.$thumb->color_code; ?>', 'favadd');" />
														...
														<div class="info" style="margin-top:10px;">
															...
															<?php
															/**********
															 * Price is hidden on satelite sites
															 */
															...
															<div class="prices info-product-price <?php echo $price_class; ?>">
																<div class="price">
																	<span itemprop="price" <?php echo $thumb->custom_order === '3' ? 'style="text-decoration:line-through;"' : '';?>>
																		 ...
																		 * Wholeslae price
																		 */
																		if ($this->session->userdata('user_cat') == 'wholesale' OR $this->session->userdata('admin_sales_loggedin')) $price = number_format($thumb->wholesale_price, 2);
																		/**********
																		 * Retail price
																		 */
														...
														<?php
														$loopend = microtime(TRUE) - $loopstart;
														echo '<br />End loop - '.($loopend);
														?>
										...
										<div class="browseoptions browseoptions-btm clearfix">
											...
											if ($this->uri->uri_string() != 'search_products')
											{
												$this->load->view($this->config->slash_item('template').'product_thumbs_filter');
											}
											else
											{
												echo '<p style="margin:0 0 0 5px;">Search results for: <strong>'.$prod_no_string.'</strong></p>';
											}
											?>
										...
										<?php //$this->load->view($this->config->slash_item('template').'sales/product_thumbs_footer_text'); ?>
